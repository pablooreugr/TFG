...
\subsubsection{Algoritmo de Richardson-Lucy}
...
\begin{enumerate}
    ...
    \item Luego se convoluciona ese resultado con la PSF girada (que en el código es la función \textit{girarPSF}, equivalente a $p^*$). Con esto se calcula la corrección que hay que aplicar.
...
Otro problema que se ha comentado anteriormente, el algoritmo solo detecta señales positivas. Por lo tanto es inválido para señales que contengan valores tanto positivos como negativos. Es el caso de la señal de Stokes $V$, que es necesario deconvolucionar para poder extraer el campo magnético. 
