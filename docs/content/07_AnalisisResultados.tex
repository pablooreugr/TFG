...
Se evidencia que la componente de Stokes $V$, al poseer una amplitud intrínseca reducida, resulta extremadamente sensible al ruido. Ninguno de los algoritmos clásicos de deconvolución logra recuperar satisfactoriamente la señal original. El método que presenta un comportamiento marginalmente superior es el filtro de Wiener, del cual se examina su mapa bidimensional para un nivel de ruido del $2\%$ (elegido arbitrariamente para ilustrar el efecto). El resultado es el siguiente:
...
Se verifica analíticamente la constancia del índice SSIM: el filtro de Wiener logra mitigar efectivamente el ruido, pero fracasa en la recuperación de la estructura subyacente de la señal original, introduciendo un sobresuavizado drástico. \\

En consecuencia, este análisis justifica la inviabilidad de deconvolucionar la señal de Stokes $V$ mediante enfoques clásicos. Por esta razón fundamental, se emplea el método propuesto en este Trabajo de Fin de Grado, el cual permite derivar topológicamente el campo magnético eludiendo la deconvolución directa de la señal de Stokes $V$.
...
Como se muestra en la figura, el número de iteraciones requerido para minimizar el RMSE siempre es mayor que el necesario para optimizar el SSIM. Esto implica que el algoritmo requiere más pasos para mitigar el ruido estadístico, pero a partir de cierto límite también comienza a sobresuavizar la imagen, limpiando el ruido a costa de difuminar la estructura, un efecto homólogo al observado en la componente $V$ en la figura (\ref{fig:visualizacionStokesV}).
