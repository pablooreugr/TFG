...
Debido a su reducida amplitud intrínseca, la componente de Stokes $V$ resulta extremadamente sensible al ruido. Esto provoca que ninguno de los algoritmos clásicos de deconvolución consiga recuperar la señal original de forma satisfactoria. El método que ofrece mejores resultados es el filtro de Wiener. Para ilustrar visualmente este efecto, a continuación se muestra el mapa bidimensional recuperado por este filtro asumiendo un nivel de ruido del 2%:
...
En consecuencia, este análisis justifica la inviabilidad de deconvolucionar la señal de Stokes $V$ mediante enfoques clásicos. Por esta razón fundamental, se emplea el método propuesto en este TFG, el cual permite derivar el campo magnético eludiendo la deconvolución directa de la señal de Stokes $V$.
...
Como se puede observar, el método lograr recuperar la estuctura original del campo magnético. Las regiones vuelven a ser más definidas y muchas estructuras vuelven a aparecer. Además hay información de picos de intensidad que vuelven a recuperarse, lo cual es un éxito. Por último argumentar que hay ciertos bulbos que no se han recuperado perfectamente. Esto es debido a que al emborronar la imagen, hemos perdido información de la señal original, por lo que el método no puede recuperarla. Pero aún así, el resultado es muy satisfactorio y demuestra que el método es capaz de recuperar la estructura del campo magnético a partir de la señal de Stokes $I$ y $V$. 
...
Como se muestra en la figura, el número de iteraciones requerido para minimizar el RMSE siempre es mayor que el necesario para optimizar el SSIM. Esto implica que el algoritmo requiere más pasos para mitigar el ruido estadístico, pero a partir de cierto límite también comienza a sobresuavizar la imagen, limpiando el ruido a costa de difuminar la estructura, un efecto parecido al observado en la componente $V$ en la figura (\ref{fig:visualizacionStokesV}).
