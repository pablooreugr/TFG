...
v_orig_1d = V_3D[:, py, px]
v_deg_1d = V_deg[:, py, px]
v_deco_1d = V_deco[:, py, px]

print("Amplitud V original:", v_orig_1d.max() - v_orig_1d.min())
print("Amplitud V degradado:", v_deg_1d.max() - v_deg_1d.min())
print("Amplitud V reconstruido:", v_deco_1d.max() - v_deco_1d.min())

print("Nivel de fondo original (extremos):", v_orig_1d[0], v_orig_1d[-1])
print("Nivel de fondo reconstruido (extremos):", v_deco_1d[0], v_deco_1d[-1])
print("Asimetria original (max vs abs(min)):", v_orig_1d.max(), v_orig_1d.min())
print("Asimetria reconstruida (max vs abs(min)):", v_deco_1d.max(), v_deco_1d.min())
